...
const Report = () => {
  ...
  return (
    <div className="overflow-x-auto mt-5">
      ...
      {/* Table Wrapper */}
      <div className="mt-6 overflow-x-auto rounded-lg border border-gray-700 bg-[#1e1e2f] shadow-lg">
        <table className="min-w-full divide-y divide-gray-700 text-sm">
          <thead className="bg-[#2a2a3d]">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-100 whitespace-nowrap">
                Title
              </th>
              {fields.map(field => (
                <th
                  key={field}
                  className="px-4 py-3 text-sm font-medium text-gray-300 text-center capitalize whitespace-nowrap"
                >
                  {field}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-800">
            {reports.map((report, rowIndex) => (
              <tr
                key={report._id}
                className="hover:bg-[#2e2e44] transition-colors duration-200"
              >
                ...
                {fields.map(field => {
                  ...
                  return (
                    <td
                      key={field}
                      className="px-4 py-3 text-center text-gray-300 whitespace-nowrap"
                    >
                      <label className="flex items-center justify-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
